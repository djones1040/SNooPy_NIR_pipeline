...
# HACK D. Jones
import NIR_ubertemp as ubertemp            # a template class that contains these two
...
class sn(object):
   ...
   def __init__(self, name, source=None, ra=None, dec=None, z=0):
      ...
      self.__dict__['model'] = model.EBV_NIR_model(self)
      ...
      self.EBVgal = 0.0     # Milky Way color excess
      self.e_EBVgal = 0.0  # ARTURO: uncertainty on Milky Way color excess.
      ...
      for band in bands:
         ...
         if dokcorr:
            if band not in self.ks_tck:
               raise RuntimeError, "No k-corrections defined for %s.  \
Either set dokcorr=0 or run self.kcorr() first" % band
   ...
   def get_e_EBVgal(self, calibration='SF11'): # ARTURO: My function
      '''Gets the value of the uncertainty in E(B-V) due to Milky Way galactic
      extinction.  The ra and decl member varialbles must be set beforehand.'''
      if self.ra is not None and self.decl is not None:
         self.e_EBVgal = dust_getval.get_dust_sigma_RADEC(self.ra, self.decl,
               calibration=calibration)
      else:
         print "Error:  need ra and dec to be defined, e_E(B-V)_gal not computed"
   def get_zcmb(self):
   ...
   def summary(self, out=sys.stdout, textFileOut=None):
      ...
      for band in self.restbands:
         print >> out, "   Observed %s fit to restband %s" % (band, self.restbands[band])
      ...
      if textFileOut is not None:
         # Verbose_text.close() # Close any previous file
         Verbose_text = open(textFileOut, 'w')
         Verbose_text.write('#     %s   \n'%self.name)
         Verbose_text.write('#-------------------\n')
         Verbose_text.write('%-13.7f  0.0        0.0  # z_helio +/ error \n'%(self.z))
         Verbose_text.write('%-13.7f  0.0        0.0  # z_CMB +/ error  \n'%(self.zcmb))
         Verbose_text.write('%-13.6f  %.6f  0.0  # (RA, DECL) \n'%(self.ra, self.decl))
         Verbose_text.write('%-13.7f  %.7f  0.0  # E(B-V) Milky Way \n'%(
                            self.EBVgal, self.e_EBVgal))
         Verbose_text.write('#-----Fitted parameters----------\n')

         for param in self.parameters:
            if self.parameters[param] is not None:
               line = "%-13.7f  %.7f" % (self.parameters[param],self.errors[param])
               if param in systs and systs[param] is not None:
                   line += "  %.7f # (sys)" % systs[param]
               else:
                   line += "  0.0  # (sys)"

               # Write the name of the parameters at the end of the line:
               line += "  # %s"%(param)

               Verbose_text.write(line+' \n')
         dist_mu_int = "%-13.7f  %.7f  0.0       # DM (H0=72.0,Om0=0.28,Ode0=0.72)\n"%(
            self.get_distmod(H0=72.0, Om0=0.28, Ode0=0.72), self.e_DM)
         Verbose_text.write(dist_mu_int)

         dist_mu_int = "%-13.7f  %.7f  0.0       # DM (H0=72.0,Om0=0.3,Ode0=0.7)\n"%(
            self.get_distmod(H0=72.0, Om0=0.3, Ode0=0.7), self.e_DM)
         Verbose_text.write(dist_mu_int)
         # Write the distance modulus based on a given cosmological model
         dist_mu_int = "%-13.7f  %.7f  0.0       # DM (H0=73.24,Om0=0.28,Ode0=0.72)\n"%(
            self.get_distmod(H0=73.24, Om0=0.28, Ode0=0.72), self.e_DM)
         Verbose_text.write(dist_mu_int)

         Verbose_text.write('#-------------------\n')
         for band in self.restbands:
            Verbose_text.write('#   Observed %-3s fit to restband %-3s | %3s_zp = %.7f, \
%3s_zp = %.7f \n' % (band, self.restbands[band], band, fset[band].zp,
                self.restbands[band], fset[self.restbands[band]].zp))
