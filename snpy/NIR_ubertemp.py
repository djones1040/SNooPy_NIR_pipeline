...
template_bands = NIRtemp.template_bands + \
				 ['u','B','V','g','r','i']
#dm15temp2.template_bands + \
...
class template:
   ...
   def __getattr__(self, name):
	  ...
	  elif name in ['u','B','V','g','r','i',
					'eu','eB','eV','eg', 'er','ei']:
		 return self.Ct.__dict__[name]
	  elif name in ['Y','J','H','K','eY','eJ','eH','eK']:
		  return self.Nt.__dict__[name]
   ...
   def mktemplate(self, dm15, dm15_int=None, dm15_colors='int', generate=0):
	  ...
	  self.Nt.mktemplate(**args2)
	  
   def domain(self, band):
	  if band in ['Bs','Vs','Rs','Is']:
		 return self.Pt.domain(band)
	  elif band in ['u','B','V','g','r','i']:
		 return self.Ct.domain(band)
	  elif band in ['Y','J','H','K']:
		 return self.Nt.domain(band)
   ...
   def eval(self, band, times, z=0, mag=1, sextrap=1, gen=1, toff=True,
	  ...
	  if band in ['Bs','Vs','Rs','Is']:
		 ...
		 return(self.Nt.eval(band, times, z, mag, sextrap, gen, toff))
...
class stemplate:
   ...
   def mktemplate(self, st, dm15_int=None, dm15_colors='int', generate=0):
	  ...
	  self.Nt.mktemplate(**args)
	  
   def domain(self, band):
	  if band in ['u','B','V','g','r','i']:
		 return self.Ct.domain(band)
	  elif band in ['Y','J','H','K']:
		 return self.Nt.domain(band)
	  elif band in ['UVW1','UVW2','UVM2']:
